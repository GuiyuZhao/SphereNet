...
def s2_grid(n_alpha, n_beta):
    ...
    '''
    beta = np.linspace(start=0, stop=np.pi, num=n_beta, endpoint=False) + np.pi / n_beta / 2  #纬向
    # ele = np.arcsin(np.linspace(start=0, stop=1, num=n_beta / 2, endpoint=False) + 1 / n_beta / 4)
    # beta = np.concatenate([np.sort(-ele), ele])
    alpha = np.linspace(start=0, stop=2 * np.pi, num=n_alpha, endpoint=False) + np.pi / n_alpha  #经向
    B, A = np.meshgrid(beta, alpha, indexing='ij')
    B = B.flatten() #展平，返回一个一维数组
    A = A.flatten()
    grid = np.stack((B, A), axis=1)  #沿着新轴连接数组
    return grid
...
def get_voxel_coordinate(radius, rad_n, azi_n, ele_n):  #坐标转换 球空间笛卡尔坐标系
...
def sphere_query(pts, new_pts, radius, nsample):
    ...
    pts = pts.contiguous()  #断开这两个变量之间的依赖，深拷贝
    new_pts = new_pts.contiguous()
    group_idx = pnt2.ball_query(radius, nsample, pts, new_pts)
    mask = group_idx[:, :, 0].unsqueeze(2).repeat(1, 1, nsample) #采样出来的第一个点，与该点一样的替换为中心点
    ...
    new_points = new_points.permute([0, 2, 3, 1]) #维度换位
...
def Rotation_invariance_weight(local_cor):
    ...
    Z_axis = torch.symeig(cov_matrix, eigenvectors=True)[1][:, :, 0]  #计算特征值（和向量），这里得到他的最大的特征向量
...
def Rotation_invariance(local_cor, local_weight=None):
    ...
    LRF_direction = torch.linalg.eigh(cov_matrix)[1]  # 计算特征值（和向量），这里得到他的最大的特征向量
...
def query_ball_point_with_number(radius, nsample, xyz, new_xyz):
    ...
    sqrdists = square_distance(new_xyz, xyz)  # 记录S个中心点（new_xyz）与所有点（xyz）之间的欧氏距离
    ...
    # repeat 会将Tensor 在指定的维度方向上进行重复。比如设置参数是 2, 3, 4: 表示在 0 维方向上 重复2次，1 维方向上重复 3次， 2 维方向上重复4次。
    group_first = group_idx[:, :, 0].view(B, S, 1).repeat([1, 1, nsample])  # 如果半径内的点没那么多，就直接用第一个点来代替了。。。
    mask = group_idx == N  # 返回满足条件的索引
...
def query_ball_point_only_number(radius, xyz, new_xyz):
    ...
    B, N, C = xyz.shape
    _, S, _ = new_xyz.shape
    group_idx = torch.zeros(N, dtype=torch.bool).to(device).view(1, 1, N).repeat([B, S, 1])   #（B, S, N）
    sqrdists = square_distance(new_xyz, xyz)  # 记录S个中心点（new_xyz）与所有点（xyz）之间的欧氏距离
    group_idx[sqrdists < radius ** 2] = 1
    number=torch.sum(group_idx,dim=2)  #group_idx is the number
...
def Determine_direction(local_cor):
    ...
    # if num_x_pos>=num_x_neg:
    #     points_x = local_cor[:, :, 0]
    # else:
    #     points_x = -local_cor[:, :, 0]
    # if num_y_pos>=num_y_neg:
    #     points_y = local_cor[:, :, 1]
    # else:
    #     points_y = -local_cor[:, :, 1]
    # if num_z_pos>=num_z_neg:
    #     points_z = local_cor[:, :, 2]
    # else:
    #     points_z = -local_cor[:, :, 2]
