...
import time
import os
from ThreeDMatch.Test.tools import get_pcd, get_keypts, get_desc, loadlog, read_log_file
...
def register2Fragments(id1, id2, keypts, descs, resultpath, desc_name='SphereNet'):
    ...
    source_keypts = keypts[0]
    target_keypts = keypts[1]
    source_desc = descs[0]
    target_desc = descs[1]
...
if __name__ == '__main__':
    ...
    for scene in scene_list:
        ...
        keyptspath = f"../../data/3DMatch/keypoints/3DMatch_{num_keypoints}_prob/{scene}/"
        ...
        for idx,i in enumerate(test_pairs):
            pcd = get_pcd(pcdpath, 'cloud_bin_' + str(i['test_pair'][0]))
            pcd_src=np.array(pcd.points).astype(np.float32)
            pcd = get_pcd(pcdpath, 'cloud_bin_' + str(i['test_pair'][1]))
            pcd_tgt = np.array(pcd.points).astype(np.float32)
            keypoints_src = np.load(os.path.join(keyptspath, f'src_{idx}.npy'))
            keypoints_tgt = np.load(os.path.join(keyptspath, f'tgt_{idx}.npy'))
            desc_src = np.load(os.path.join(descpath, 'cloud_bin_' + str(i['test_pair'][0]) + '_'+str(i['test_pair'][1]) + '_0.desc.SphereNet.bin.npy'))
            desc_tgt = np.load(os.path.join(descpath, 'cloud_bin_' + str(i['test_pair'][0]) + '_' + str(i['test_pair'][1]) + '_1.desc.SphereNet.bin.npy'))
            keypoints = [keypoints_tgt,keypoints_src]
            descs=[desc_src,desc_tgt]
            num_inliers, inlier_ratio, gt_flag = register2Fragments(i['test_pair'][0], i['test_pair'][1], keypoints, descs, resultpath,
                                                                    desc_name)
        ...
        result = []
        for idx,i in enumerate(test_pairs):
            line = read_register_result(i['test_pair'][0], i['test_pair'][1])
    ...
    print(f"All 8 scene, average recall (for every fragments): {average_recall_fragments}%")
