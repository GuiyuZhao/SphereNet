...
class ContrastiveLoss(nn.Module):
    ...
    def calculate_loss(self, dists, pids):  # mean
        # 加一个测试参数来调正样本和负样本的关系？可以和epoch有关？
        # 对于负样本 平均好还是取最小值好？
        # 描述符紧凑性
        # 增加描述子的维度
        ...
        furthest_positive, _ = torch.max(dists * same_identity_mask.float(), dim=1)     # 找到差距最大的anchor-positive pair
        closest_negative, _ = torch.min(dists + 1e5 * same_identity_mask.float(), dim=1)  # 找到最相近的negative值
        diff = furthest_positive - closest_negative   # 最差的positive小鱼最好的negative
    ...
    def calculate_loss_mean2(self, dists, pids):  # mean2 和mean对于loss的计算是一样的，但正确率的计算不一致
        # 加一个测试参数来调正样本和负样本的关系？可以和epoch有关？
        # 对于负样本 平均好还是取最小值好？
        # 描述符紧凑性
        # 增加描述子的维度
        ...
        # dists * same_identity_mask get the distance of each valid anchor-positive pair.
        mean_positive= torch.mean(dists * same_identity_mask.float(), dim=1)  # 找到差距最大的anchor-positive pair
        mean_negative= torch.mean(dists * (1-same_identity_mask.float()), dim=1)  # 找到最相近的negative值
        diff = mean_positive - mean_negative  # 最差的positive小鱼最好的negative
        accuracy = (diff < 0).sum() * 100.0 / diff.shape[0]
        loss_positive=torch.max((dists - self.pos_margin) * same_identity_mask.float(), torch.zeros_like(dists))
        loss_negative=torch.max((self.neg_margin - dists)*(1-same_identity_mask.float()), torch.zeros_like(dists))
        loss = torch.mean(loss_positive)+torch.mean(loss_negative)
